...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-db-"));
  ...
  const adminUrl = `postgres://creator:creator@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "creator");
  return `postgres://creator:creator@127.0.0.1:${port}/creator`;
}
