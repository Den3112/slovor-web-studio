...
async function migrationStatementAlreadyApplied(
  ...
  const createTableMatch = statement.match(/create\s+table\s+(?:if\s+not\s+exists\s+)?(?:"([^"]+)"|'([^']+)'|([^\s;]+))/i);
  if (createTableMatch) {
    const tableName = createTableMatch[1] ?? createTableMatch[2] ?? createTableMatch[3];
    const exists = await tableExists(sql, tableName);
    // console.log(`DEBUG RECONCILE - Table ${tableName} exists: ${exists}`);
    return exists;
  }
...
export async function applyPendingMigrations(url: string): Promise<void> {
  ...
  console.log("APPLYING PENDING MIGRATIONS:", state.status, state.pendingMigrations);
