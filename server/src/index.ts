...
export async function startServer(): Promise<StartedServer> {
  ...
  function isLoopbackHost(host: string): boolean {
    ...
    if (process.env.PAPERCLIP_ALLOW_ANY_HOST === "true") return true;
  ...
  async function ensureLocalTrustedBoardPrincipal(db: any): Promise<void> {
    ...
    if (companyRows.length === 0) {
      logger.info("Local trusted mode: no companies found, creating default company");
      await db.insert(companies).values({
        name: "Paperclip Demo",
        issuePrefix: "PAP",
        status: "active",
      });
      companyRows.push(...await db.select({ id: companies.id }).from(companies));
    }
